...
criterion_triplet = TripletLoss(alpha=.4)
    ...
    * num_training
    ...
    for step, ((X_anchor, X_positive, X_negative), (y_anchor, y_positive, y_negative)) in enumerate(train_loader):
        ...
        if (step+1)%10 == 0:
            ...
            #vis.log_scalars({"tag":"train_cls_loss", "scalar_value":cls_loss, "global_step":step+1})
            #vis.log_scalars({"tag":"train_tri_loss", "scalar_value":tri_loss, "global_step":step+1})
            #vis.log_scalars({"tag":"train_acc", "scalar_value":acc, "global_step":step+1})
            ...
            if test_loader:
                ...
                print("Test after steps[{}], cls_loss: {:.6f}, acc: {:.6f}, best_acc: {:.6f}".format(step+1, cls_loss, acc, best_acc if best_acc else acc))

                vis.log_scalar({"tag":"test_cls_loss", "scalar_value":cls_loss, "global_step":step+1})
                vis.log_scalar({"tag":"test_acc", "scalar_value":acc, "global_step":step+1})
            """ 
        ...
        if (step+1) == config.STEPS:
            if test_loader:
                ...
                print("Test after steps[{}], cls_loss: {:.6f}, acc: {:.6f}".format(step+1, cls_loss, acc))

                #vis.log_scalars("Test", dict(test_acc=acc), global_step=step+1)
                #vis.log_scalar({"tag":"test_acc", "scalar_value":acc, "global_step":step+1})
            break
